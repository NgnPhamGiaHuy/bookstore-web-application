...
class UserPageController {
    async index(req, res, next) {
        try {
            ...
            res.render('User/user', {
                customerData, totalQuantity, countryOptions, updateSuccess, numberOfOrders: 0, numberOfWishlist: 0
            });
        } catch (error) {
            next(error);
        }
    ...
    async update(req, res, next) {
        try {
            const customerId = req.session.customerId;
            const {customer_firstName, customer_lastName, day, month, year, gender, nationality, phoneNo, email} = req.body;
